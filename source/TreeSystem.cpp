...
void TreeSystem::placeTrees(int biome)
{
    ...
    for (int t = 0; t < MAX_TREES; t++)
    {
        ...
        int x = 4 + GetRandomValue(0, 41);
        int z = 4 + GetRandomValue(0, 41);
        ...
        float worldX = 100.0f * x + 50.0f;
        float worldZ = 100.0f * z + 50.0f;
        ...
        float posX = x * 100.0f + 50.0f;
        float posY = h - 1.0f; // на 1 единицу ниже поверхности
        float posZ = z * 100.0f + 50.0f;

        // Поворот (точно по DBP: rnd(359))
        float rotY = (float)GetRandomValue(0, 359);

        // Масштаб (точно по DBP, конвертируем проценты в множители)
        float sc = (95.0f + GetRandomValue(0, 10)) / 100.0f; // 0.95 - 1.05
        float scaleX = sc;
        float scaleY = sc;
        float scaleZ = sc;
        ...
        case 2:                                            // Mountains: вытянуты по Y
            sc = (95.0f + GetRandomValue(0, 15)) / 100.0f; // 0.95 - 1.10
            scaleX = sc;
        ...
        case 3: // Desert: случайная высота
            sc = (95.0f + GetRandomValue(0, 10)) / 100.0f;
            scaleX = sc;
            scaleY = sc + (GetRandomValue(0, 40) - 20) / 100.0f;
            scaleZ = sc;
            break;
        ...
        case 6: // Moon: сильно варьируется высота
            sc = (95.0f + GetRandomValue(0, 10)) / 100.0f;
            scaleX = sc;
            scaleY = sc - 0.30f + GetRandomValue(0, 60) / 100.0f; // 0.65 - 1.25
            scaleZ = sc;
            break;
        }
